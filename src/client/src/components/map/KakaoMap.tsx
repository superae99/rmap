...
const KakaoMap: React.FC<KakaoMapProps> = ({
  ...
  }, [isKakaoLoaded, latitude, longitude, level]);
    ...
    markers.forEach(markerData => {
      ...
      window.kakao.maps.event.addListener(marker, 'click', (event: any) => {
        ...
        if (markerData.content && infoWindowRef.current) {
          const content = `
            <div style="padding: 15px; min-width: 320px; max-width: 400px; min-height: 200px; position: relative; overflow: visible;">
              <style>
                .kakao-infowindow-content img { display: none !important; }
                .kakao-infowindow-content { 
  ...
  }, [isKakaoLoaded, latitude, longitude, level, markers, areas, showAreaBounds]);

  // 로딩 중이거나 에러가 있을 때
  if (!isKakaoLoaded || loadingError) {
    return React.createElement('div', {
      ...
      loadingError ? 
        React.createElement('div', { style: { textAlign: 'center' } },
          React.createElement('div', { style: { fontSize: '18px', marginBottom: '8px' } }, '⚠️ 지도 로드 실패'),
          React.createElement('div', { style: { fontSize: '14px' } }, loadingError)
        ) :
        React.createElement('div', { style: { textAlign: 'center' } },
          React.createElement('div', { style: { fontSize: '18px', marginBottom: '8px' } }, '🗺️ 지도 로딩 중...'),
          React.createElement('div', { style: { fontSize: '14px' } }, '카카오맵 API를 불러오고 있습니다.')
        )
    );
  }
