...
import { exportFilteredPartnersToExcel, downloadPartnerTemplate, readPartnersFromExcel } from '../utils/excelUtils'
...
const PartnersPage = () => {
  ...
  // 전체 교체 업로드 핸들러
  const handleReplaceAllUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    if (!file) return

    // 관리자 권한 체크
    if (!user || (user.account !== 'admin' && user.account !== 'hyeongraekim')) {
      alert('❌ 관리자만 전체 데이터 교체가 가능합니다.')
      return
    }

    // 확인 대화상자
    if (!confirm('⚠️ 경고: 기존 모든 거래처 데이터가 삭제되고 새 데이터로 교체됩니다.\n이 작업은 되돌릴 수 없습니다. 계속하시겠습니까?')) {
  ...
  return React.createElement('div',
    ...
    React.createElement('div',
      ...
      React.createElement('div',
        ...
        React.createElement('div',
          ...
          // 전체 교체 업로드 버튼 (관리자만)
          user && (user.account === 'admin' || user.account === 'hyeongraekim') && React.createElement('label',
            {
              style: {
                padding: '10px 16px',
                backgroundColor: uploading ? '#999' : '#f44336',
                color: 'white',
                border: 'none',
                borderRadius: '6px',
            ...
            uploading 
              ? React.createElement('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: '200px' } },
                  React.createElement('div', { style: { marginBottom: '4px', fontSize: '12px' } },
                    `⏳ ${uploadMessage || '업로드 중...'}`
                  ),
                  uploadProgress > 0 && React.createElement('div', 
                    { style: { width: '100%', height: '4px', backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: '2px', overflow: 'hidden' } },
                    React.createElement('div', {
                      style: {
                  ...
                  uploadProgress > 0 && React.createElement('div', { style: { fontSize: '11px', marginTop: '2px' } },
            ...
            React.createElement('input',
              {
                type: 'file',
                accept: '.xlsx,.xls',
                onChange: handleReplaceAllUpload,
                disabled: uploading,
                style: { display: 'none' }
              }
            )
    ...
    // 업로드 결과 모달
    showUploadModal && uploadResult && React.createElement('div',
      {
        style: {
          position: 'fixed',
          top: 0,
      ...
      React.createElement('div',
        ...
        React.createElement('div',
          { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' } },
          React.createElement('h3', { style: { margin: 0 } }, '전체 교체 업로드 결과'),
          React.createElement('button',
            {
        ...
        React.createElement('div',
          ...
          React.createElement('div',
            ...
            React.createElement('div', { style: { fontWeight: 'bold', marginBottom: '10px' } }, uploadResult.message),
            React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' } },
              React.createElement('div', null, `✅ 성공: ${uploadResult.created}개`),
              React.createElement('div', null, `❌ 실패: ${uploadResult.failed}개`)
            )
          )
        ),
        // 오류 목록 표시
        uploadResult.errors && uploadResult.errors.length > 0 && React.createElement('div',
          null,
          React.createElement('h4', { style: { margin: '0 0 10px 0' } }, '오류 목록:'),
          React.createElement('div',
            { 
              style: { 
                maxHeight: '200px', 
            ...
            ...uploadResult.errors.map((error: any, index: number) =>
              React.createElement('div',
                { 
                  ...
                  style: { 
                    padding: '5px 0',
                    borderBottom: index < uploadResult.errors.length - 1 ? '1px solid #eee' : 'none',
                    fontSize: '14px'
                  }
                },
                `${error.partnerCode || '알 수 없음'}: ${error.error}`
              )
            )
          )
        ),
