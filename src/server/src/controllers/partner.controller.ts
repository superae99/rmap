...
export const updatePartner = async (req: Request & { user?: any }, res: Response) => {
  try {
    ...
    if (managerChangedDate !== undefined && !currentManagerEmployeeId) partner.managerChangedDate = managerChangedDate ? new Date(managerChangedDate) : null as any
...
export const replaceAllPartners = async (req: Request & { user?: any }, res: Response) => {
  try {
    const { partners } = req.body
    ...
    try {
      ...
      await queryRunner.manager.delete(Partner, {})
      ...
      for (let batchStart = 0; batchStart < partners.length; batchStart += batchSize) {
        ...
        if (batchPartnersToInsert.length > 0) {
          try {
            await queryRunner.manager.insert(Partner, batchPartnersToInsert)
            ...
            for (let i = 0; i < batchPartnersToInsert.length; i++) {
              try {
                const partner = queryRunner.manager.create(Partner, batchPartnersToInsert[i])
